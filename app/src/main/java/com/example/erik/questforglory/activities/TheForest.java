...
public class TheForest extends AppCompatActivity {
    ...
    TextView youAreDead;
    TextView accept;
    TextView sprigganDefeatedGrid;
    TextView sprigganDefeated;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sprigganDefeatedText = "Spriggan Defeated!\nRewards:\n" + Math.round(spriggan.getXPYield()) + " XP\n" + Math.round(spriggan.getHerbYield()) + " Herbs\n" + Math.round(spriggan.getGoldYield()) + " Gold";
        ...
        sprigganDefeated.setText(sprigganDefeatedText);
    ...
    public void checkDamagePotion() {
        if(damagePotion.getCurrentDuration() > 0 && player.isAlive() && spriggan.isAlive() && damagePotionDurationCooldown.getVisibility() == View.VISIBLE) {
            damagePotion.tickDuration();
            damagePotionDurationCooldownText = "Duration: " + Math.round(damagePotion.getCurrentDuration());
            damagePotionDurationCooldown.setText(damagePotionDurationCooldownText);
        }
        if(damagePotion.getCurrentDuration() < 1 && player.isAlive() && spriggan.isAlive()) {
            damagePotionDurationCooldown.setVisibility(View.INVISIBLE);
    ...
    public void checkHealthPotion() {
        if(healthPotion.getCurrentDuration() > 0 && player.isAlive() && spriggan.isAlive() && healthPotionDurationCooldown.getVisibility() == View.VISIBLE) {
            healthPotion.tickDuration();
            healthPotionDurationCooldownText = "Duration: " + Math.round(healthPotion.getCurrentDuration());
            healthPotionDurationCooldown.setText(healthPotionDurationCooldownText);
        }
        if(healthPotion.getCurrentDuration() < 1 && player.isAlive() && spriggan.isAlive()) {
            healthPotionDurationCooldown.setVisibility(View.INVISIBLE);
    ...
    public void checkSpriggan() {
        if(Math.round(spriggan.getHealth()) <= 0) {
            spriggan.setAlive(false);
            spriggan.setHealth(Math.round(spriggan.getMaxHealth()));
            player.resetBattleDefense();
            player.increaseXP(Math.round(spriggan.getXPYield()));
            player.increaseHerbs(Math.round(spriggan.getHerbYield()));
            player.increaseGold(Math.round(spriggan.getGoldYield()));

            gearPieceReward = new GearPiece(0, "gear piece", "rarity", "stats", 1, 50, 1, 5, 0, 0, 160, 120, 40);

            if(spriggan.getLevel() > 1) {
                for (int i = 1; i < spriggan.getLevel(); i++) {
            ...
            sprigganDefeatedGrid.setVisibility(View.VISIBLE);
            sprigganDefeated.setVisibility(View.VISIBLE);
    ...
    public void Continue(View view) {
        sprigganDefeatedGrid.setVisibility(View.INVISIBLE);
        sprigganDefeated.setVisibility(View.INVISIBLE);
    ...
    public void checkCharge() {
        if(player.isAlive() && spriggan.isAlive() && chargeDurationCooldown.getVisibility() == View.INVISIBLE) {
            if(player.getBattleDefense() >= spriggan.getDamage()) {
                player.setBattleDefense(spriggan.getDamage());
            }
        ...
        if(player.isAlive() && spriggan.isAlive() && chargeDurationCooldown.getVisibility() == View.VISIBLE) {
            ...
            if(charge.getCurrentDuration() > 0 && player.isAlive() && spriggan.isAlive()) {
                charge.tickDuration();
                chargeDurationCooldownText = "Duration: " + Math.round(charge.getCurrentDuration());
                chargeDurationCooldown.setBackgroundResource(R.drawable.border_quest);
                chargeDurationCooldown.setText(chargeDurationCooldownText);
            ...
            if(charge.getCurrentCooldown() < 0 && player.isAlive() && spriggan.isAlive()) {
                chargeDurationCooldown.setVisibility(View.INVISIBLE);
                chargeImage.setImageAlpha(255);
                charge.setOnDurationCooldown(false);
                charge.refresh();
    ...
    public void checkMend() {
        if(mendDurationCooldown.getVisibility() == View.VISIBLE) {
            if(mend.getCurrentDuration() > 0 && player.isAlive() && spriggan.isAlive()) {
                mend.tickDuration();
            ...
            if(mend.getCurrentDuration() < 1 && player.isAlive() && spriggan.isAlive()) {
                mend.tickCooldown();
                mendDurationCooldownText = "Cooldown: " + Math.round(mend.getCurrentCooldown() + 1);
                mendDurationCooldown.setBackgroundResource(R.drawable.border_level_xp);
                mendDurationCooldown.setText(mendDurationCooldownText);
            }
            if(mend.getCurrentCooldown() < 0 && player.isAlive() && spriggan.isAlive()) {
                mendDurationCooldown.setVisibility(View.INVISIBLE);
                mendImage.setImageAlpha(255);
                mend.setOnDurationCooldown(false);
                mend.refresh();
    ...
    public void checkDefenseUp() {
        if(defenseUpDurationCooldown.getVisibility() == View.VISIBLE) {
            if(defenseUp.getCurrentDuration() > 0 && player.isAlive() && spriggan.isAlive()) {
            ...
            if(defenseUp.getCurrentDuration() < 1 && player.isAlive() && spriggan.isAlive()) {
                defenseUp.tickCooldown();
                defenseUpDurationCooldownText = "Cooldown: " + Math.round(defenseUp.getCurrentCooldown() + 1);
                defenseUpDurationCooldown.setBackgroundResource(R.drawable.border_level_xp);
                defenseUpDurationCooldown.setText(defenseUpDurationCooldownText);
            }
            if(defenseUp.getCurrentCooldown() < 0 && player.isAlive() && spriggan.isAlive()) {
                defenseUpDurationCooldown.setVisibility(View.INVISIBLE);
                defenseUpImage.setImageAlpha(255);
                defenseUp.setOnDurationCooldown(false);
                defenseUp.refresh();
    ...
    public void useStrike(View view) {
        if (player.isAlive() && spriggan.isAlive()) {
            spriggan.takeDamage(player.getBattleDamage() * strike.getDamage() / 100);
            sprigganHealthText = "  Health: " + Math.round(spriggan.getHealth()) + " / " + Math.round((spriggan.getMaxHealth())) + "  ";
            sprigganHealth.setText(sprigganHealthText);
            checkSpriggan();
            checkDamagePotion();
    ...
    public void useCharge(View view) {
        if (player.isAlive() && spriggan.isAlive() && chargeDurationCooldown.getVisibility() == View.INVISIBLE) {
            spriggan.takeDamage(player.getBattleDamage() * charge.getDamage() / 100);
            sprigganHealthText = "  Health: " + Math.round(spriggan.getHealth()) + " / " + Math.round(spriggan.getMaxHealth()) + "  ";
            sprigganHealth.setText(sprigganHealthText);
            chargeImage.setImageAlpha(125);
            chargeDurationCooldown.setVisibility(View.VISIBLE);
    ...
    public void useMend(View view) {
        if (player.isAlive() && spriggan.isAlive() && mendDurationCooldown.getVisibility() == View.INVISIBLE) {
            mendImage.setImageAlpha(125);
            mendDurationCooldown.setVisibility(View.VISIBLE);
    ...
    public void useDefenseUp(View view) {
        if (player.isAlive() && spriggan.isAlive() && defenseUpDurationCooldown.getVisibility() == View.INVISIBLE) {
            defenseUpImage.setImageAlpha(125);
            defenseUpDurationCooldown.setVisibility(View.VISIBLE);
    ...
    public void useHealthPotion(View view) {
        if(healthPotion.getAmount() > 0 && player.getHealth() != player.getMaxHealth() && player.isAlive() && spriggan.isAlive() && healthPotionDurationCooldown.getVisibility() == View.INVISIBLE) {
            healthPotionImage.setImageAlpha(125);
            healthPotionDurationCooldown.setVisibility(View.VISIBLE);
    ...
    public void useDamagePotion(View view) {
        if (damagePotion.getAmount() > 0 && player.isAlive() && spriggan.isAlive() && damagePotionDurationCooldown.getVisibility() == View.INVISIBLE) {
            damagePotionImage.setImageAlpha(125);
            damagePotionDurationCooldown.setVisibility(View.VISIBLE);
